...
2. 生成 .cursor/rules/{{project_name}}.mdc — AI 开发军规
...
[先验知识注入]
请静默读取以下文件，建立全局认知：
1. {{output_dir}}Legacy_SignInCenter_Claims.md — 旧文档声称
2. {{output_dir}}01_Module_Skeleton_and_Stack.md — 骨架与技术栈
3. {{output_dir}}02_External_Contracts.md — 对外契约
4. {{output_dir}}03_Downstream_Dependencies.md — 下游依赖
5. {{output_dir}}04_Data_Model_and_Lifecycle.md — 数据模型
6. {{output_dir}}05_Business_Orchestration.md — 业务编排
7. {{output_dir}}06_Async_Jobs_and_Compensation.md — 异步与补偿
8. {{output_dir}}07_Config_and_Observability.md — 配置与可观测
9. {{output_dir}}08a~08e_Module_*.md — 业务模块深潜
...
### 任务 B：生成 .cursor/rules/{{project_name}}.mdc
...
### 任务 C：生成 .gemini/rules/{{project_name}}.md (Antigravity 兼容规则)

基于任务 B 整理出的所有协议和红线内容，生成一份完全相同的规则文件，但必须严格符合 Gemini CLI 的项目级规则标准。

**存放路径**：`.gemini/rules/{{project_name}}.md`
...
```yaml
---
description: {{project_display_name}} 核心意图路由与防御性编码军规
globs: **/*
trigger: always_on
---
```
...
trigger: always_on
---
# {{project_display_name}} 开发军规

（此处无缝粘贴任务 B 中生成的第一章到第五章的所有内容）
```

[Action]
1. 在 {{output_dir}} 目录下生成 00_Master_Catalog.md
2. 在 .cursor/rules/ 目录下生成 {{project_name}}.mdc
3. 在 .gemini/rules/ 目录下生成 {{project_name}}.md

[Constraint - 工业级底线]

**工作区边界**：所有文件访问必须限定在当前工作区根目录下。若 `{{output_dir}}` 下的前序产出文件不存在，必须列出缺失文件并停止，输出：`❌ 未在工作区找到 [文件路径]，请确认前序步骤已完成。` 禁止推断替代内容，禁止访问工作区外目录。

**输出格式锁定**：
- 00_Master_Catalog.md 必须包含：项目概览、文件导航、场景路由表、关键发现汇总
- {{project_name}}.mdc 必须包含：意图路由、防御性编码红线、知识库双写协议、项目特有约束、盲区行为规范
...
> - 产出文件：00_Master_Catalog.md + .cursor/rules/{{project_name}}.mdc + .gemini/rules/{{project_name}}.md
