...
[先验知识注入]
请静默读取以下文件，建立先验认知：
1. {{output_dir}}01_Module_Skeleton_and_Stack.md — §4 配置坐标
2. {{output_dir}}06_Async_Jobs_and_Compensation.md — §8 衍生约束（格式参考）
如有旧文档：{{output_dir}}Legacy_{{project_name}}_Claims.md — §7 风险声称、§8 待确认项
...
**区域 A：配置文件**
- {{project_name}}-start/src/main/resources/ — 全部配置文件
- {{project_name}}-start/src/test/resources/ — 测试配置（对比差异）
...
- {{project_name}}-service/src/main/java/**/infrastructure/repository/ — 带缓存逻辑的 Repository
- {{project_name}}-service/src/main/java/**/infrastructure/configuration/ — 缓存配置类
...
**区域 E：Spring 配置类**
- {{project_name}}-service/src/main/java/**/infrastructure/configuration/ — 全部配置类
...
[Action]
在 {{output_dir}} 目录下生成 07_Config_and_Observability.md
...
**工作区边界**：所有文件访问必须限定在当前工作区根目录下。若 `{{project_name}}-start/src/main/resources/` 等扫描路径在工作区内不存在，必须立即停止并输出：`❌ 未在工作区找到 [路径]，无法完成审计。请确认 PROJECT_NAME 与目标项目一致。` 禁止推断替代路径，禁止访问工作区外目录。
