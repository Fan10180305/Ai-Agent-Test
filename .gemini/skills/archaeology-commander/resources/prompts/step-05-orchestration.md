...
[先验知识注入]
请静默读取以下文件，建立先验认知：
1. {{output_dir}}02_External_Contracts.md — §1 Dubbo 接口清单（这是链路入口）
2. {{output_dir}}03_Downstream_Dependencies.md — §2 实际调用点追踪
3. {{output_dir}}04_Data_Model_and_Lifecycle.md — §3 查询模式矩阵
如有旧文档：{{output_dir}}Legacy_{{project_name}}_Claims.md — §4 业务链路声称
...
**区域 A：应用层入口（app 模块）**
- {{project_name}}-app/src/main/java/**/rpc/ — 全部 RPC 实现类
- 逐文件读取至 EOF

**区域 B：业务编排层（service 模块 core/）**
- {{project_name}}-service/src/main/java/**/core/ — 全部子包
- 重点扫描：DomainService、BizService、Executor、Handler、Checker、Strategy 等关键类

**区域 C：基础设施层（service 模块 infrastructure/）**
- {{project_name}}-service/src/main/java/**/infrastructure/ 下的：
  - repository/ — 仓储实现
  - component/ — 组件
  - configuration/ — 配置
...
- {{project_name}}-service/src/main/java/**/common/ — 工具类、通用 DTO、扩展
...
**文件一**：在 `{{output_dir}}` 下生成 `05_Business_Orchestration.md`，包含：
- RPC 入口映射表（完整）
- 核心业务链路（精简格式，每条 ≤ 15 行）
- DDD 分层审计汇总
- 设计模式识别
...
**文件二（脚本接口，必须产出）**：在 `{{output_dir}}` 下生成 `05_module_manifest.json`。
...
**工作区边界**：所有文件访问必须限定在当前工作区根目录下。若 `{{project_name}}-app/` 或 `{{project_name}}-service/` 在工作区内不存在，必须立即停止并输出：`❌ 未在工作区找到 [路径]，无法完成审计。请确认 PROJECT_NAME 与目标项目一致。` 禁止推断替代路径，禁止访问工作区外目录。
