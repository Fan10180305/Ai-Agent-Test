...
已完成步骤：
- Step 01: 骨架测绘 — 已确认核心工具与执行模型
- Step 03: 下游依赖 — 已记录超时与容错配置
- Step 04: 数据模型 — 已记录 TypeHandler 信息与状态中转模型
- Step 05: 业务编排 — 已发现流水线接力逻辑
- Step 06: 异步机制 — 已记录后台心跳与信号处理机制
...
2. ai-knowledge-knowledge/06_Async_Jobs_and_Compensation.md
如有旧文档：ai-knowledge-knowledge/Legacy_ai-knowledge_Claims.md — §7 风险声称
...
请 read_file 读取 `ai-knowledge-knowledge/07_Config_and_Observability.md` ，将其作为「旧假说」参照：
...
- 扫描全项目对 `config.json` 的 `jq` 读取点

**区域 B：环境变量与 Shell 配置**
- 扫描脚本中的变量定义与导出 (export)
- 扫描 `.gitignore` 对敏感文件的保护
...
**区域 D：日志与监控**
- 扫描 `run-archaeology.sh` 中的日志重定向与 Stdout 拦截逻辑
- 扫描对 `transcript.jsonl` 的实时监控逻辑

### 提取任务

#### 1. 配置项决策清单 (JSON 字段、环境变量、脚本参数)

#### 2. 缓存/临时状态全景 (.tmp/ 目录管理、心跳文件 TTL)

#### 3. 错误码/退出码字典 (exit 0/1/2... 的含义)

#### 4. 异常处理体系 (set -e, trap EXIT, || exit 1)

#### 5. 日志规范分析 (Stdout vs Logfile vs Transcript)

#### 6. 衍生约束清单 (关于环境依赖、配置校验与日志原子性的约束)

[Action]
在 ai-knowledge-knowledge/ 目录下生成 07_Config_and_Observability.md
...
- WRITE_TARGET: ai-knowledge-knowledge/07_Config_and_Observability.md
...
> - 扫描范围：config.json + 核心脚本变量 + 日志拦截逻辑
> - 提取结果：[X] 个业务配置项、[Y] 组退出码字典
> - 衍生约束：[D] 条（🔴强制 / 🟡建议）
> - 旧文档差异：❌不符 [A] 条 / 🆕新发现 [B] 条 / ✅其余已验证
> - EOF 状态：已确认遍历至最后一行，无静默截断

> [!RELAY] 定向审计约束
> - **物理事实 (Context)**: [本步发现的、对模块深潜分析有决定性影响的配置事实]
