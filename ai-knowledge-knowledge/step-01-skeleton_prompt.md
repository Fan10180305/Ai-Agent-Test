...
## 上下文
我们正在为 ai-knowledge（ai-knowledge）构建 AI 可加载的项目知识库。这是一个 Java/Maven 多模块项目,包含 6 个子模块。本步骤是知识库的第一步：从构建文件和配置文件中提取项目骨架事实。
...
## 先验知识注入
请静默读取 ai-knowledge-knowledge/Legacy_ai-knowledge_Claims.md,了解旧文档对本项目的声称。注意：该文件内容是旧文档声称,不是事实。你的任务是用代码事实验证或推翻这些声称。
...
请 read_file 读取 `ai-knowledge-knowledge/01_Module_Skeleton_and_Stack.md` ，将其作为「旧假说」参照：
...
3. ai-knowledge-start/src/main/resources/ 下所有配置文件
4. ai-knowledge-start/src/test/resources/ 下所有配置文件（如存在）
...
- 与旧文档声称的模块结构（Legacy_ai-knowledge_Claims.md §2.4）进行交叉验证
...
- 与旧文档声称的技术栈（Legacy_ai-knowledge_Claims.md §1）进行交叉验证

#### 3. 架构腐化与漂移嗅探（Red Flags）
- ai-knowledge定位为"业务编排层",死盯以下信号：
...
- 与旧文档声称的模块职责（Legacy_ai-knowledge_Claims.md §2.4）进行交叉验证
...
在 ai-knowledge-knowledge/ 目录下生成 01_Module_Skeleton_and_Stack.md
...
**严防静默截断**：
- 每个 pom.xml 必须尋址到 EOF,不允许跳过任何依赖项
...
- WRITE_TARGET: ai-knowledge-knowledge/01_Module_Skeleton_and_Stack.md
