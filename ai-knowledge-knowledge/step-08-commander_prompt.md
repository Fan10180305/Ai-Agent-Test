...
[Role] 业务语义深度解读专家。
你的任务是为 commander 模块生成业务语义深度解读文档——
...
- **物理事实 (Context)**: 流水线高度依赖 `jq` 解析 `config.json`，且 `CURSOR_CMD` 存在跨平台硬编码风险。
- **推演约束 (Constraint)**: Step 08 深潜时，需特别注意不同模块（Commander vs Archaeology vs Infrastructure）在读写 `$OUTPUT_DIR` 时的路径竞争风险。
- **物理锚点 (Anchors)**: `run-archaeology.sh` L35 (config 加载), L61 (LOG_DIR 定义), L518 (Cursor 调用)。
...
本轮相关约束：当前为模块深潜（Commander），需审计主控逻辑、参数分发及各模块间的路径竞争风险。
...
[Task: commander 模块深度解读]

### 扫描范围

**核心类清单**（从 Step 05 提取）：
- `main()`: 协调整条流水线的生命周期 (run-archaeology.sh)
- `preflight()`: 环境检查与协议挂载 (run-archaeology.sh)
- `init_commander()`: 初始化 001 指挥官 AI (run-archaeology.sh)
- `run_step()`: 执行原子考古步骤 (run-archaeology.sh)
- `ask_commander()`: 驱动认知接力 (run-archaeology.sh)
