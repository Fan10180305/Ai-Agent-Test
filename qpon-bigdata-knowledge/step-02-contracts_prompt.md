...
Step02 must scan all DAG entry create_* calls, ExternalTaskSensor edges, airflow_config signatures; BQ project oppo-gcp-prod-digfood-129869 asia-southeast2; authority=dags/ only; shared entry create_composer_bq_task/create_external_sensor
...
本步骤聚焦 qpon-bigdata-client 模块，提取对外契约的完整事实。

已知关键线索（由 001 根据 Step 01 产出动态注入）：
> [!SUCCESS] 骨架测绘闭环验证
> - 扫描范围：父 POM(N/A) + 0 个子模块 POM + 33 个 DAG 包（1167 .py）+ 共享库 airflow_config/qpon_metadata + Variable/Connection
> - 提取结果：识别了 18 项中间件/第三方依赖，22 项配置坐标
> - 架构预警：捕获了 9 项潜在问题（Red Flags）
> - 旧文档验证：NO_DOCS
> - WRITE_TARGET: qpon-bigdata-knowledge/01_Module_Skeleton_and_Stack.md
> - WRITE_RESULT: UPDATED
> - WRITE_BYTES: 16767
> - WRITE_SHA256: 781059b935112474b9f0d39cb16ed3a00c90e022713c4a248e6d471b93a1dd82
...
[先验知识注入]
请静默读取以下文件，建立先验认知：
1. qpon-bigdata-knowledge/01_Module_Skeleton_and_Stack.md — §5 子模块职责定义
如有旧文档：qpon-bigdata-knowledge/Legacy_qpon-bigdata_Claims.md — §2 对外接口声称


[Task: 对外契约全量提取]

请集中算力扫描 qpon-bigdata-client 模块的全部 Java 源码，执行以下提取：

### 扫描范围（严格限定）
qpon-bigdata-client/src/main/java/ 下所有 .java 文件，逐文件读取至 EOF。
...
[Action]
在 qpon-bigdata-knowledge/ 目录下生成 02_External_Contracts.md
...
**工作区边界**：所有文件访问必须限定在当前工作区根目录下。若 `qpon-bigdata-client/src/main/java/` 在工作区内不存在，必须立即停止并输出：`❌ 未在工作区找到 [路径]，无法完成审计。请确认 PROJECT_NAME 与目标项目一致。` 禁止推断替代路径，禁止访问工作区外目录。
...
> [!SUCCESS] 对外契约测绘闭环验证
> - 扫描范围：qpon-bigdata-client/src/main/java/ 下全部 [N] 个 .java 文件
