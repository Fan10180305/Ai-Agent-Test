...
Step08 metadata: drill decision/failure modes; do not redo Step05; dags/ only; next metadata: do not assume export layer digested hour-Skip (alarm_h still daily-sensor); distinguish ES raise-red vs swallow-green; separate 2999 read/write semantics; GenAI fallback and alarm datet
...
[Role] 业务语义深度解读专家。
你的任务是为 元数据与GCP监控 模块生成业务语义深度解读文档——
...
[Action]
在 qpon-bigdata-knowledge/ 目录下生成 08l_Module_元数据与GCP监控.md
