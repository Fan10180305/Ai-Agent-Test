...
Step08 ods-h: drill decision/failure modes; do not redo Step05; dags/ only; next: verify waited ODS tasks still live-registered; do not assume order MERGE ON id is cross-shard safe; sidepath failure may still show Airflow green
...
[Role] 业务语义深度解读专家。
你的任务是为 ODS小时批贴源层 模块生成业务语义深度解读文档——
...
[Task: ODS小时批贴源层 模块深度解读]

### 扫描范围

**核心类清单**（从 Step 05 提取）：
- - `qpon_ods_h/qpon_ods_h.py`
- `qpon_ods_h`
- `qpon_dim_h/qpon_dim_h.py`
- `qpon_dim_h`
- `create_external_task_skip_sensor_hour`
...
[Action]
在 qpon-bigdata-knowledge/ 目录下生成 08c_Module_ODS小时批贴源层.md
