...
Step04: build table entities from backtick FQN; separate source vs layer datasets; ES index field maps; feishu live path only; dags/ only
...
本步骤聚焦 qpon-bigdata-dao 模块，以及分库分表配置。

已知关键线索（由 001 根据前序步骤动态注入）：
> [!SUCCESS] 下游依赖测绘闭环验证
> - BQ FQN 1062 / dataset 35；活工厂 751；Sensor 441；Cloud Run ES 写22+删1
> - WRITE_TARGET: qpon-bigdata-knowledge/03_Downstream_Dependencies.md
> - WRITE_RESULT: UPDATED
> - WRITE_BYTES: 23596
> - WRITE_SHA256: 3d57b6037e9e87ccb49e19092e3f0e615d54c94a2fa7c6ac993c4edc11eaadec
> - NO_CHANGE_REASON: N/A
> [!RELAY] Step04: 以 backtick FQN 建表实体；区分源库 dataset 与分层 dataset；ES index 字段映射；飞书仅活路径；禁止 scripts/
...
[先验知识注入]
请静默读取以下文件，建立先验认知：
1. qpon-bigdata-knowledge/01_Module_Skeleton_and_Stack.md — §2 依赖雷达（ShardingSphere）、§4 配置坐标（数据库连接）
2. qpon-bigdata-knowledge/02_External_Contracts.md — §3 枚举定义清单（作为状态机字段枚举值的参考来源）
如有旧文档：qpon-bigdata-knowledge/Legacy_qpon-bigdata_Claims.md — §3 数据模型声称、§8 待确认项
...
**区域 A：实体类（必扫）**
- qpon-bigdata-dao/src/main/java/**/entity/ 下所有 .java 文件
- 逐文件读取至 EOF

**区域 B：MyBatis Mapper（必扫）**
- qpon-bigdata-dao/src/main/java/**/mapper/ 下所有 Mapper 接口（.java）
- qpon-bigdata-dao/src/main/resources/mapper/ 下所有 Mapper XML（.xml）
- 逐文件读取至 EOF

**区域 C：配置与基础设施（必扫）**
- qpon-bigdata-dao/src/main/java/**/configuration/ — 数据源配置类
- qpon-bigdata-dao/src/main/java/**/handler/ — MyBatis TypeHandler
- qpon-bigdata-dao/src/main/java/**/typehandler/ — 类型处理器
- qpon-bigdata-start/src/main/resources/ 和 src/test/resources/ 中与数据库相关的配置
...
[Action]
在 qpon-bigdata-knowledge/ 目录下生成 04_Data_Model_and_Lifecycle.md
...
**工作区边界**：所有文件访问必须限定在当前工作区根目录下。若 `qpon-bigdata-dao/` 在工作区内不存在，必须立即停止并输出：`❌ 未在工作区找到 [路径]，无法完成审计。请确认 PROJECT_NAME 与目标项目一致。` 禁止推断替代路径，禁止访问工作区外目录。
